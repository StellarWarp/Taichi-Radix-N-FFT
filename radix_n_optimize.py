...
def fft_pass(gtid: ti.u32,
    ...
    if ti.static(is_pow2_size):
        ...
        offset = gtid//t_size * t_size * R_

    radix_buffer = ti.Vector([[0 for x in range(4)]for r in range(2*R_)],
                              dt=ti.f32)
    
    k_,p_ = (gtid%t_size)//P_, (gtid%t_size)%P_
    for r in range(R_):
        for x in ti.static(range(4)):
            radix_buffer[r,x] = buffer[not target, P_*R_*k_ + p_ + P_*r + offset][x]
        
    # radix_buffer,ofs = fft_subpass(radix_buffer, k_,size,P_,R_, inverse)
    ...
    for _ in range(log2_radix):
        ...
        for i in range(S):
            ...
            if inverse:
                twiddle.y = -twiddle.y 
            f1 = vec4_arr_get(radix_buffer, src[0] + r_ofs)
            f2 = vec4_arr_get(radix_buffer, src[1] + r_ofs)
            f2 = tm.vec4(tm.cmul(f2.xy, twiddle), tm.cmul(f2.zw, twiddle))
            v1 = f1 + f2
            v2 = f1 - f2
            for x in range(4):
                radix_buffer[dst[0] + w_ofs,x] = v1[x]
                radix_buffer[dst[1] + w_ofs,x] = v2[x]
        w_ofs,r_ofs = r_ofs,w_ofs
    

    for r in range(R_):
        buffer[target,gtid%t_size + r*t_size + offset] = vec4_arr_get(radix_buffer, r + r_ofs)
...
def fft_grouped(size: ti.template(),
    ...
    (radix, log2_radix,
     radix_prod, log_2_radix_prod,
     non_pow2_pass, pow2_pass) = ti.static(radix_seqence(size, 1))
    ...
    for k in range(pixel_size):
        target = target_
        for step in ti.static(range(0,total_pass)):
            fft_non_pow2_pass(k, size,
                              radix[step],
                              radix_prod[step],
                              buffer, target, inverse)
    ...
    #     target = not target
    # ti.loop_config(block_dim=size)
    # for gtid in range(pixel_size):
    #     target = target_
    #     if gtid % radix[0] == 0:
    #         fft_pass(gtid//radix[0], size, log2_size, is_pow2_size,
    #                     radix[0], log2_radix[0],
    #                     radix_prod[0], log_2_radix_prod[0],
    #                     buffer, target, inverse)
    #     target = not target
    # ti.loop_config(block_dim=size)
    # for gtid in range(pixel_size):
    ...
    #         if gtid % radix[step] == 0:
    #             fft_pass(gtid//radix[step], size, log2_size, is_pow2_size,
    #                         radix[step], log2_radix[step],
    #                         radix_prod[step], log_2_radix_prod[step],
    #                         buffer, target, inverse)
    #         target = not target
...
def main(kernel_profiler=False, debug=False):
    ...
    def gui_thread():
        ...
        fft_x_dir_radix,_,_,_,_,_ = radix_seqence(fft_size_x,5)
        fft_y_dir_radix,_,_,_,_,_ = radix_seqence(fft_size_y,5)
