    ...
    for i in range(pixel_size):
        ...
        for s in range(log2_size):
            ...
            ev = fast_mod(w + i, size) + \
                fast_mul(fast_div(i, log2_size), log2_size) # multiple target offset
            od = ev + b
            phi = -2 * tm.pi / size * w
...
def image_mul(freq: ti.template(), frequency_filter: ti.template(), scale: ti.f32):
    ...
    '''
    for i, j in frequency_filter:
        freq[i, j, 0].xy = tm.cmul(freq[i, j, 0].xy, frequency_filter[i, j]) * scale
        freq[i, j, 0].zw = tm.cmul(freq[i, j, 0].zw, frequency_filter[i, j]) * scale
        freq[i, j, 1].xy = tm.cmul(freq[i, j, 1].xy, frequency_filter[i, j]) * scale
        freq[i, j, 1].zw = tm.cmul(freq[i, j, 1].zw, frequency_filter[i, j]) * scale
...
def image_blend(src: ti.template(), target: ti.template()):
    for i, j in target:
        ...
        if target.dtype == ti.u8:
            target[i, j] = min(target[i, j] + min(src[src_i,src_j]*256,255),255)
        else:
            # linear add
...
def ACES_tonemap(img: ti.template(), adapted_luminance: ti.f32):
    for i, j in img:
        if img.dtype == ti.u8:
            ...
            img[i, j] = (c * (2.51 * c + 0.03)) / (c * (2.43 * c + 0.59) + 0.14) * 256
        else:
            c = img[i, j] * adapted_luminance
            img[i, j] = (c * (2.51 * c + 0.03)) / (c * (2.43 * c + 0.59) + 0.14)
...
def field_transpose(src: ti.template(), dst: ti.template()):
    if dst.dtype == ti.u8 and (src.dtype == ti.f32 or src.dtype == ti.f16):
        for i, j in dst:
            dst[i, j] = ti.min(src[j * src.shape[0] // dst.shape[1], i * src.shape[1] // dst.shape[0]] * 256, 255)
    else:
        for i, j in dst:
            dst[i, j] = src[j * src.shape[0] // dst.shape[1], i * src.shape[1] // dst.shape[0]]
...
def freq_visualize(freq: ti.template(),transposed: ti.template()):
    ...
    for i, j in transposed:
        ...
        transposed[i_, j_].xy = tm.log2(tm.vec2(tm.length(freq[i_freq, j_freq, 0].xy), tm.length(freq[i_freq, j_freq, 0].zw)) + 1) / 10
        transposed[i_, j_].zw = tm.log2(tm.vec2(tm.length(freq[i_freq, j_freq, 1].xy), tm.length(freq[i_freq, j_freq, 1].zw)) + 1) / 10
...
def main(kernel_profiler=False,debug=False):
    ...
    render_taget = ti.Vector.field(4, dtype=ti.u8, shape=(img_size_y, img_size_x))
    transposed   = ti.Vector.field(4, dtype=ti.u8, shape=(img_size_x, img_size_y))
    #fft
    rgba = ti.Vector.field(4, dtype=ti.f32, shape=(fft_size_y, fft_size_x))
    rgba_freq = ti.Vector.field(4, dtype=ti.f32, shape=(fft_size_y, fft_size_x, 2))
    buffer = ti.Vector.field(4, dtype=ti.f32, shape=(2, fft_size_x * fft_size_y))
    # for blur the image
    frequency_fiter = ti.Vector.field(2, dtype=ti.f32, shape=(fft_size_y, fft_size_x))
    ...
    def gui_thread():
        ...
        window = ti.ui.Window('Real Time FFT Image Convolution', (img_size_x, img_size_y),fps_limit=200,)
        gui = window.get_gui()
        ...
        while window.running:
            ...
            window.show()
            t3 = time.time()
            image_copy_time = weighted_average(image_transpose_time + t2-t1,image_copy_time)
    ...
    try:
        ...
        while program_running:
            ...
            if old_filter_x_scale != filter_x_scale or old_filter_y_scale != filter_y_scale:
                compute_frequency_filter(frequency_fiter, rgba, buffer, filter_x_scale, filter_y_scale)
            ...
            t_f2 = time.time()
            between_fft_ifft_time = weighted_average(t_f2 - t_f1, between_fft_ifft_time)
            if use_numpy_fft:
